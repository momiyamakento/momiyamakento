<!-- アニメーションヘッダー（青） -->
<p align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&color=0A3D91&height=220&section=header&text=Kento%20MOMIYAMA&fontSize=55&fontColor=ffffff&animation=fadeIn&fontAlignY=38"/>
</p>

<!-- タイピングエフェクト -->
<p align="center">
  <img src="https://readme-typing-svg.herokuapp.com?font=Fira+Code&weight=600&size=24&pause=1200&color=0A3D91&center=true&vCenter=true&width=500&lines=Software+Developer;UI%2FUX+Designer;Product+Design+%C3%97+AI+%C3%97+App+Development;Future+University+Hakodate+Student" />
</p>

---

## 💫 About Me
- 🎓 Future University Hakodate / 情報アーキテクチャ学科  
- 🚀 LOGROKU サブリーダー（ミライケータイプロジェクト2025）  
- 👶 EMOKA（保育士業務改善）学内ハッカソン **優勝**  
- ☎ TelephoneTool（職場効率化アプリ）をFlutter + Firebaseで開発  
- 🧭 企画 / UIUX / 体験設計 / Firebase設計 / PM  
- 🧑‍💻 **Python / C / Java / HTML / CSS / Flutter / Dart / Firebase / Firestore / PostgreSQL**  
- 🎨 **Figma / Miro / Git / GitHub**



---

## 🎨 Tech Stack
<p align="center">

<!-- Languages -->
<img src="https://skillicons.dev/icons?i=python,c,java,html,css" /><br>

<!-- Mobile / Backend -->
<img src="https://skillicons.dev/icons?i=flutter,dart,firebase,postgresql" /><br>

<!-- Tools -->
<img src="https://skillicons.dev/icons?i=git,github,figma,miro" />

</p>

---

## 🚀 Featured Projects

### 🎲 LOGROKU – AI × 日記 × すごろく  
> 日記を自動で“すごろく化”し、人生の成長を可視化するアプリ。  
> 1年間のチーム開発 / UIUX・Firebase設計・PM担当。  
> ※本番版はPrivate。こちらは紹介用リポジトリです。

👉 https://github.com/momiyamakento/LOGUROKU_introduction

---

### ☎ TelephoneTool  
> アルバイト先の課題から誕生した、連絡先管理＆業務効率化ツール。  
> Flutter × Firebase で開発。

👉 https://github.com/momiyamakento/TelephoneTool

---

### 👶 EMOKA – 感情 × 保育士業務改善（優勝）  
> 保育士の“記録作業”を削減し、保護者と感情を共有できる共育サービス。  
> UX調査 / 体験設計 / UIデザイン / スライド作成を担当。

👉 https://github.com/momiyamakento/emoka_introduction

---

### 💬 TrendFlash – Hakodate Instagram Trends  
> 函館のInstagramトレンドを収集し、人気ワードを可視化するWebアプリ。  
> Google Maps API × 可視化 UI × AI

👉 https://github.com/p2hacks2024/pre-05



<p align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&color=0A3D91&height=150&section=footer"/>
</p>
